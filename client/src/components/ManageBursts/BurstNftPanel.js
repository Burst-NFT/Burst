...
import { useBurstAssets } from '../queries';
...
function BurstNftPanel({ data, setAlert }) {
  ...
  const { isLoading, data: burstAssets } = useBurstAssets({ assets });
  ...
  return (
    <>
      ...
      <Accordion>
        ...
        <Divider />
        <AccordionActions>
          <Button size='small' color='error' onClick={() => setDestroyDialogOpen(true)}>
            Destory
          </Button>
